...
export default abstract class AbstractServiceLocal {
    ...
    protected get<T>(url: string): Promise<T> {
        ...
        console.log("🚀 ~ file: AbstractServiceLocal.tsx:60 ~ AbstractServiceLocal ~ session:", session, this.sessionEnabled)
    ...
    protected getBody<B,T>(url: string, body: B): Promise<T> {
        let session: string;
        if (this.sessionEnabled) {
            session = this.checkForSession();
        } else {
        ...
        console.log("🚀 ~ file: AbstractServiceLocal.tsx:76 ~ AbstractServiceLocal ~ options:", options)
    ...
    private checkForSession(): string {
        ...
        console.log("🚀 ~ file: AbstractServiceLocal.tsx:274 ~ AbstractServiceLocal ~ checkForSession ~ session:", session)
        if (!session) {
    ...
    private getSession(): string | undefined {
        ...
        console.log("🚀 ~ file: AbstractServiceLocal.tsx:284 ~ AbstractServiceLocal ~ getSession ~ sessionStorage:", sessionStorage)
        if (!!session) {
            return session;
        } else {
            return undefined;
