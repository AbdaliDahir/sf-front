...
class NotesHistoryV2 extends React.Component<Props, State> {

    private DATETIME_FORMAT = process.env.REACT_APP_FASTR_DATETIME_FORMAT;
    private externalAppsSettings = BlocksExternalAppsConfig.history.vegasCouriers;
    ...
    public formatNotesByContact = () => {
        ...
        const allNotes = _.sortBy(this.props.notes, note => note.creationDate).reverse();
            ...
            return (
                <li key={index} className="text-wrap">
                    ...
                    <Row className={"mx-1"}>
                        ...
                        <Col sm={2} id={`creator-${this.props.caseId}-${index}`}>
                            {firstNoteOfGroup.creator ? firstNoteOfGroup.creator.login : ""}
                            <UncontrolledTooltip target={`creator-${this.props.caseId}-${index}`} placement={"right"}>
                        ...
                        <Col sm={6} className="notes-description-container">
                            {groupedNote.notes.map((note) => <div
                                className={`description px-2`} style={{maxWidth: "250px"}}
    ...
    public renderDescription = (note: CaseNote, noteIndex: number): JSX.Element => {
        return note.type === 'COURIER' ?
            <ExternalLinksBlock settings={this.externalAppsSettings} clientContext={this.props.client} idParams={{ idsrcDem: note.description }} isLoading={true} >
                <span style={{color: 'blue', cursor: 'pointer'}}>Indexation de la demande VEGAS {note.description}</span>
            </ExternalLinksBlock> :
            <span>{this.truncateNote(note, noteIndex)}{this.displayEllipsisIfNecessary(note, noteIndex)}</span>;
    };
...
export default connect(mapStateToProps)(NotesHistoryV2)
