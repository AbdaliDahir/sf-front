...
class EditClientCategoryV2 extends Component<Props, State> {
    ...
    public async componentDidMount() {
        ...
        for (const service of client.clientData!.services) {
        ...
        this.setState({serviceMap, serviceCategories, isVip: client.clientData!.vip})
    }
    ...
    public catchServiceCategoryChange = (serviceId, formWasChanged: React.FormEvent<HTMLInputElement>) => {
        if (formWasChanged && formWasChanged.currentTarget) {
            ...
            const originalServiceCategory = this.props.client.clientData!.services.find(service => service.id === serviceId)!.billingAccount.serviceCategory;
    ...
    public renderServices() {
        if (this.props.client && this.props.client.clientData!.services) {
            const {services} = this.props.client.clientData!;
                    ...
                    <tr key={service.id}
                        ...
                        <td className="no-margin-formgroup">
                            <FormSelectInput bsSize="sm" name={`editClientCategory.serviceCategory.${service.id}`}
                                             ...
                                             validations={{isRequired: ValidationUtils.notEmpty}}><span
...
export default connect(mapStateToProps, mapDispatchToProps)(EditClientCategoryV2)
