...
import {Card, CardBody, Col, Container, CustomInput, FormGroup, Label, Row, Tooltip} from "reactstrap";
...
class DuplicateBillingsV2 extends React.Component<Props, State> {
    ...
    private isOwnerPerson = !!this.props.client.clientData?.ownerPerson
    ...
    public async componentDidMount() {
        ...
        this.props.client.clientData?.services.forEach((service) => {
        ...
        const contactAddress: AddressModel = {
            civility: this.isOwnerPerson ? this.props.client.clientData?.ownerPerson.civility : "",
            firstName: this.isOwnerPerson ? this.props.client.clientData?.ownerPerson.firstName : "",
            lastName: this.isOwnerPerson ? this.props.client.clientData?.ownerPerson.lastName : "",
            companyName: this.isOwnerPerson ? "" : this.props.client.clientData?.ownerCorporation.name,
            streetWithNumber: this.isOwnerPerson ? this.props.client.clientData?.ownerPerson.address.address1 : this.props.client.clientData?.ownerCorporation.address.address1,
            additionalAdress1: this.isOwnerPerson ? this.props.client.clientData?.ownerPerson.address.address2 : this.props.client.clientData?.ownerCorporation.address.address2,
            additionalAdress2: undefined,
            postalCode: this.isOwnerPerson ? this.props.client.clientData?.ownerPerson.address.zipcode : this.props.client.clientData?.ownerCorporation.address.zipcode,
            city: this.isOwnerPerson ? this.props.client.clientData?.ownerPerson.address.city : this.props.client.clientData?.ownerCorporation.address.city,
            country: this.isOwnerPerson ? this.props.client.clientData?.ownerPerson.address.country : this.props.client.clientData?.ownerCorporation.address.country,
        }
        this.setState({
            billingAccountId,
    ...
    public getListOfBillingAccountIds = (): JSX.Element[] => {
        ...
        this.state.billingAccountIds.forEach((value, key) => {
            value.forEach(clientNb => {
                if (clientNb === this.props.client.service?.label) {
    ...
    public getListOfBillingType = (): JSX.Element[] => {
        ...
        if (billTypes) {
            billTypes.filter(billType => billType.usedByDuplicata && this.isMobile ? billType.category === "MOBILE" : billType.category === "FIXE")
                .forEach((billType) => {
                    billingTypeOptions.push(<option key={billType.code}
                                                    value={billType.code}>{billType.label}</option>)
                })
        }
    ...
    public searchBillings = async () => {
        const bills = await this.billingService.getBillingInfo(this.state.billingAccountIdSearch,
            ...
            this.formatDate(this.state.startDate),
            this.formatDate(this.state.endDate));
        const bankingMovements = bills.bankingMovements.filter(bankingMovement => {
            return bankingMovement.bill !== null
        })
        ...
        return (
            <div>
                <Row className={"duplicate-billing-row"}>
                    ...
                    <Col md={3}>
                        <FormGroup>
                            <FormDateInputV2 name="startDate"
                                             id="startDate"
                                             minDate={minDate}
                                             maxDate={currentDate}
                                             value={startDate}
...
export default connect(mapStateToProps, mapDispatchToProps)(DuplicateBillingsV2)