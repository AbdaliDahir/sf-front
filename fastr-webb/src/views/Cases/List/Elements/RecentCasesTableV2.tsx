...
import SelectedCaseSummaryV2 from "./SelectedCaseSummaryV2";
...
export default class RecentCasesTableV2 extends React.Component<Props, State> {
    ...
    public componentDidUpdate(prevProps: Props, prevState: State) {
        const {listCasesFromStomp, activeIndex, selectedCase} = this.state
        if (JSON.stringify(prevState.listCasesFromStomp) === JSON.stringify(listCasesFromStomp)) {
            return
        }
        if (0 === activeIndex && listCasesFromStomp[0].caseId === selectedCase!.caseId) {
            return
        }
        this.setState({activeIndex: null}, () => this.recentCaseRef.current.click())
    }
            ...
            (recentCase, index) =>
                <React.Fragment>
                    <tr key={index}
                        ...
                        ref={index === 0 ? this.recentCaseRef : null}>
                        <td>
                        ...
                        <td id={"clientRequest" + index} className={"recent-cases-table__client-request-square"}>
                            ...
                            <UncontrolledTooltip
                                popperClassName={"recent-cases-table__tooltip-popper"}
                    ...
                    <tr className={"selectedCaseSummary"}>
                        <td className="p-0" colSpan={9}>
                            <Collapse isOpen={this.state.activeIndex === index} colSpan={6}>
                                {recentCase ? <SelectedCaseSummaryV2 case={recentCase}
                                                                     fromModalMatchingCase={this.props.fromModalMatchingCase}
                                                                     idService={idService}
