...
const CollapsibleLandedOptionsTvBlock = (props: Props) => {
        ...
        if (option && option?.status == "Suspendu") {
            return (
                <React.Fragment>
                    ...
                    className="text-danger font-weight-bold ml-2">Suspendue</span>
                </React.Fragment>)
        } else {
    ...
    const renderServicesTV = (options: LandedOption[]) => {
        const landedServicesTvOptions = options.filter(options => filterByOptionStatus(options));
        if (!landedServicesTvOptions.length) {
            if (optionState === "Actif") {
                return ServiceUtils.renderEmptyDataMsg("offer.servicetv.enabled.empty", true)
            } else {
                return ServiceUtils.renderEmptyDataMsg("offer.servicetv.disabled.empty", true)
            }
        } else {
            return landedServicesTvOptions.sort((optionA, optionB) => ContractUtils.sortLandedTvOptions(optionA, optionB))
    ...
    if (service && service.optionsTv) {
        const options = [...service.optionsTv];
        const billedOptions = options.filter(e => e?.billing?.billingPrice > 0)?.length
        return (
            <Row className="border-top flex-align-middle p-2">
                ...
                <Col md={2} className={'offset-md-3'}>
                    <LoadableText isLoading={service}>
                        <h6 className="mb-0 float-right">{LocaleUtils.formatCurrency(MobileServiceUtils.computeLandedOptionsTvAmount(options))}</h6>
                    </LoadableText>
                </Col>
                <Col sm={1}>
...
export default CollapsibleLandedOptionsTvBlock
