...
class ThemeFilter extends React.Component<Props, State> {
    ...
    public getThemes(actCode) {
        ...
        if (this.props.isSupervisor) {
            this.caseService.getActionThemesFromTray(actCode).then(res =>
                this.setState({
                    themes: res
                })
            )
        } else {
            this.caseService.getActionThemesFromStock(actCode).then(res =>
                this.setState({
                    themes: res
                })
            )
    ...
    private getInitialThèmes(){
        const encodedSessionStorageFilters = sessionStorage.getItem(this.props.activityCode+SessionStorageKeys.ACTIONS)
