...
class ThemeFilter extends React.Component<Props, State> {
    ...
    public componentDidMount() {
        this.getThemes(this.props.activityCode, this.props.site);
    }
    ...
    public componentDidUpdate(prevProps: Readonly<Props>) {
        ...
        if (this.props.activityCode && this.props.activityCode !== prevProps.activityCode) {
            this.getThemes(this.props.activityCode, this.props.site);
            const encodedSessionStorageFilters = sessionStorage.getItem(this.props.activityCode+SessionStorageKeys.DOSSIERS)
    ...
    public getThemes(actCode, site) {
        if (!actCode) {
            return;
        }
        if (this.props.isSupervisor) {
            this.caseService.getThemesFromTray(actCode, site).then(res =>
                this.setState({
                    themes: res
                })
            )
        } else {
            this.caseService.getThemesFromStock(actCode).then(res =>
                this.setState({
                    themes: res
                })
            )
